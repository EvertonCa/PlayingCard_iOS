...
//  PlayingCard
//
//  Created by Everton Cardoso on 27/05/2018.
...
class PlayingCardView: UIView {
    ...
    private func drawPips()
    {
        ...
        func createPipString(thatFits pipRect: CGRect) -> NSAttributedString {
            ...
            let attemptedPipString = centeredAttibutedString(suit, fontSize: verticalPipRowSpacing)
            let probablyOkayPipStringFontSize = verticalPipRowSpacing / (attemptedPipString.size().height / verticalPipRowSpacing)
            let probablyOkayPipString = centeredAttibutedString(suit, fontSize: probablyOkayPipStringFontSize)
            if probablyOkayPipString.size().width > pipRect.size.width / maxHorizontalPipCount {
                return centeredAttibutedString(suit, fontSize: probablyOkayPipStringFontSize /
